...
assert.doesNotMatch(router, /inline for small\/simple work/i);
assert.doesNotMatch(router, /mandatory orchestrator/i);
assert.doesNotMatch(router, /coordinator-only/i);
assert.doesNotMatch(router, /do not execute implementation, research, or task work inline/i);
assert.doesNotMatch(router, /only inline work is user-facing synthesis/i);
assert.doesNotMatch(router, /Use deploy_agent to delegate all concrete work/i);
